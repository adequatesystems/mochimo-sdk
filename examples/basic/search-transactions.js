  ...
  try {
    ...
    if (data.transactions && data.transactions.length > 0) {
      ...
      if (data.next_offset !== undefined) {
        ...
        console.log(`   Run with: node examples/basic/search-transactions.js --address ${addressHex} --offset ${data.next_offset}`);
      }
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    if (!fs.existsSync('wallet-config.json')) {
      ...
      console.log('  node examples/basic/search-transactions.js --address <hex_address>');
      console.log('  node examples/basic/search-transactions.js --tx <transaction_hash>');
      process.exit(1);
    }
